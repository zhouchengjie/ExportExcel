...
namespace ExportExcel
{
    public class NPOIHelper
    {
        ...
        /// <param name="coluids">需更新的列号</param>
        /// <param name="rowid">需更新的开始行号</param>
        public static void UpdateExcel2(string outputFile, string sheetname, string[][] updateData, int rowid)
        {
            FileStream readfile = new FileStream(outputFile, FileMode.Open, FileAccess.Read);

            XSSFWorkbook xssfworkbook = new XSSFWorkbook(readfile);
            readfile.Close();
            ISheet sheet1 = xssfworkbook.GetSheet(sheetname);
            for (int i = 0; i < updateData.Length; i++)
            {
                for (int j = 0; j < updateData[i].Length; j++)
                {
                    try
                    {
                        ...
                        if (sheet1.GetRow(i + rowid).GetCell(j) == null)
                        {
                            sheet1.GetRow(i + rowid).CreateCell(j);
                        }
                        sheet1.GetRow(i + rowid).GetCell(j).SetCellValue(updateData[i][j]);
                        //sheet1.GetRow(i + rowid).GetCell(j).CellStyle.DataFormat = HSSFDataFormat.GetBuiltinFormat("0.00");
